...
interface ReadyDocumentCardProps {
  document: DocumentAnalysis
  index: number
}

/**
 * One processed-document summary card on the Ready screen, driven by the
 * real document analysis returned from the compile endpoint.
 */
export default function ReadyDocumentCard({ document: doc, index }: ReadyDocumentCardProps) {
  return (
    <div
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2">
            <span className="font-mono text-mono text-outline">
              {doc.metadata.token_count.toLocaleString()} tokens
            </span>
          </div>
        </div>
      </div>
