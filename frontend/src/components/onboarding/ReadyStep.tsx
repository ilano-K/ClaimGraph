...
import ReadyDocumentCard from './ReadyDocumentCard'
...
/**
 * Ready screen. Confirmation hero plus the real processed documents from the
 * compile response and the actions that hand off into the main workspace.
 */
      ...
      <div className="relative w-full max-w-3xl flex flex-col items-center z-10">
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <p className="font-body-lg text-body-lg text-on-surface-variant">
            Successfully processed {documents.length} document{documents.length === 1 ? '' : 's'}.
          </p>
        </div>

        <div className="w-full glass-panel rounded-xl p-6 mb-10 flex flex-col gap-4">
          {documents.map((doc, index) => (
            <ReadyDocumentCard key={doc.metadata.id} document={doc} index={index} />
          ))}
        </div>
